  ...
  const targetOffset = (e) => {
    ref?.current.addEventListener("mousemove", (e) => {
      ...
      if ((star_offsetX / base_width) * 100 <= 10) {
        ...
        ((star_offsetX / base_width) * 100 >= 10) &
        ((star_offsetX / base_width) * 100 <= 20)
      ) {
        star_width = "20%";
        star_text = "싫어요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 20) &
        ((star_offsetX / base_width) * 100 <= 30)
      ) {
        star_width = "30%";
        star_text = "재미없어요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 30) &
        ((star_offsetX / base_width) * 100 <= 40)
      ) {
        star_width = "40%";
        star_text = "별로예요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 40) &
        ((star_offsetX / base_width) * 100 <= 50)
      ) {
        star_width = "50%";
        star_text = "부족해요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 50) &
        ((star_offsetX / base_width) * 100 <= 60)
      ) {
        star_width = "60%";
        star_text = "보통이에요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 60) &
        ((star_offsetX / base_width) * 100 <= 70)
      ) {
        star_width = "70%";
        star_text = "볼만해요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 70) &
        ((star_offsetX / base_width) * 100 <= 80)
      ) {
        star_width = "80%";
        star_text = "재미있어요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 80) &
        ((star_offsetX / base_width) * 100 <= 90)
      ) {
        star_width = "90%";
        star_text = "훌륭해요";
      } else if (
        ((star_offsetX / base_width) * 100 >= 90) &
        ((star_offsetX / base_width) * 100 <= 100)
      ) {
        star_width = "100%";
        star_text = "최고예요!";
      }
