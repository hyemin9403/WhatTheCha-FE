...
const Signup = () => {
  ...
  const changeName = (e) => {
    ...
    const regExp = /^([a-zA-Z0-9ㄱ-ㅎ|ㅏ-ㅣ|가-힣]){2,16}$/i;
          ...
          <SignForm>
            ...
            >
              <Input
                ...
                _autocomplete="off"
                _value={pwd}
                _placeholder="영문, 숫자, 특문 포함 8자 이상"
                width="100%"
              />
            </InputSign>
          </SignForm>
          ...
          <SignupCheck>
            <div className="signup-check-list">
              <label
                ...
                checked={checkList.length === ids.length}
            ...
            <div className="signup-check-list">
              <label className="signup-check-label" htmlFor={ids[0]}>
                <input
                  onChange={(e) => onChangeEach(e, ids[0])}
                  checked={checkList.includes(ids[0])}
                  name={ids[0]}
                  id={ids[0]}
                  type="checkbox"
                  className="signup-check-radio"
                />
                <span className="signup-check-circle"></span>만 14세 이상입니다
              </label>
            </div>
            <div className="signup-check-list">
              <label className="signup-check-label" htmlFor={ids[1]}>
                <input
                  onChange={(e) => onChangeEach(e, ids[1])}
                  checked={checkList.includes(ids[1])}
                  name={ids[1]}
                  id={ids[1]}
                  type="checkbox"
                  className="signup-check-radio"
                />
                <span className="signup-check-circle"></span>
                <button>왓챠피디아 서비스 이용약관</button>에 동의합니다 (필수)
              </label>
            </div>
            <div className="signup-check-list">
              <label className="signup-check-label" htmlFor={ids[2]}>
                <input
                  onChange={(e) => onChangeEach(e, ids[2])}
                  checked={checkList.includes(ids[2])}
                  name={ids[2]}
                  id={ids[2]}
                  type="checkbox"
                  className="signup-check-radio"
                />
                <span className="signup-check-circle"></span>
                <button>왓챠피디아 서비스 이용약관</button>만 14세 이상입니다
              </label>
            </div>
            <div className="signup-check-list">
              <label className="signup-check-label" htmlFor={ids[3]}>
                <input
                  onChange={(e) => onChangeEach(e, ids[3])}
                  checked={checkList.includes(ids[3])}
                  name={ids[3]}
                  id={ids[3]}
                  type="checkbox"
                  className="signup-check-radio"
                />
                <span className="signup-check-circle"></span>
                <button>개인정보 수집 및 이용에 대한 안내</button>에 동의합니다
                (필수)
              </label>
            </div>
            <div className="signup-check-list">
              <label className="signup-check-label" htmlFor={ids[4]}>
                <input
                  onChange={(e) => onChangeEach(e, ids[4])}
                  checked={checkList.includes(ids[4])}
                  name={ids[4]}
                  id={ids[4]}
                  type="checkbox"
                  className="signup-check-radio"
                />
                <span className="signup-check-circle"></span>
                <button>신작 알림 이벤트 정보 수신</button>에 동의합니다 (선택)
              </label>
            </div>
          </SignupCheck>
          <button onClick={signUp} ref={submitBtn} className="btn">
            계정생성하기
          </button>
